...
const PressDetails = ({ title }) => {
    ...
    try {
      ...
      if (response.status === 200) {
        if (response.data && response.data.data) {
          ...
          const found = newData.data.find(item => item.ID === idFromUrl);
          if (found) {
  ...
  useEffect(() => {
    ...
    if (idFromUrl) {
      ...
      navigate(isPressPage ? '/press' : isElectronicDisclosurePage ? '/electronic-disclosures' : isWebcastsPage ? '/webcasts' : '/public-disclosures');
    }
  }, [idFromUrl]);
  return (
    <div className="press-details">
      <Link
        to={isPressPage ? "/press" : isElectronicDisclosurePage ? '/electronic-disclosures' : isWebcastsPage ? '/webcasts' : "/public-disclosures"}
        state={{ pageNo: pageNo }}
        className="view-all"
      >
        <FaArrowLeft />
      ...
      {list.fetching ? <div className="loader"><Loader /></div> : list.success ? isEmpty(filteredData) ? <div className="error-screen">
        <MdError />
        <p>Data not found!</p>
      </div> : <RevealAnimation>
